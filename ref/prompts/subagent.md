...
6. **한국 건축법 및 건설기준 용어 번역 원칙 (New)**: 건축법 및 KDS/KCS 기준은 외래어 용어(예: 포치, 캐노피, 발코니, 테라스 등)를 그대로 쓰지 않고 순화된 한글 법률 용어(예: 차양, 처마, 노대, 피로티 등)로 규정하는 경우가 대부분입니다. 에이전트가 도구 검색 시 외래어로 결과가 나오지 않는다면, 즉시 다음과 같은 연관 한글 법률 용어로 변경하여 검색하거나 관련 법령(예: '건축법 시행령 119') 전체를 조회한 뒤 내부 필터링 기능을 적극 활용하십시오:
   - 포치, 캐노피 (Porch, Canopy) -> '차양' 또는 '처마'
   - 발코니, 테라스 (Balcony, Terrace) -> '노대' (노대 등)
   - 필로티 (Piloti) -> '피로티' 또는 '필로티'
